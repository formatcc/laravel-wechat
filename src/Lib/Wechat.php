...
use Formatcc\LaravelWechat\Lib\Utils\Prpcrypt;
...
class Wechat
{
	...
	const MSGTYPE_SHAKEAROUND = 'ShakearoundUserShake';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_post($url,$param,$post_file=false){
		return Http::post($url, $param, $post_file);
	}
	...
	protected function setCache($cachename,$value,$expired){
		Cache::setCache($cachename,$value,$expired);
	}
	...
	protected function removeCache($cachename){
		return Cache::removeCache($cachename);
	}
     ...
     * }
     */
    public function getCardIdList($offset=0,$count=50, $status_list=array()) {
        if ($count>50)
            $count = 50;
        $data = array(
            'offset' => $offset,
